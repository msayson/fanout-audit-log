...
- [ ] **P0-05** Worker Lambda function (Kotlin): implement the worker as a Kotlin Lambda function with `architecture: ARM_64` and `snapStart: ON_PUBLISHED_VERSIONS`. Expose `putAuditEvent()` wrapping `PutRecord` with exponential backoff + jitter, retry-vs-terminal classification (throttling → retry; persistent failure → surface as unit-of-work failure), validating payload against P0-02. Deploy as a **published version** (SnapStart requires a version; the event source or alias must not point to `$LATEST`). Grant the worker role `firehose:PutRecord` on the P0-04 stream.
  *Deploy/test:* unit test covers backoff + terminal classification (mocked client); one live `PutRecord` against the deployed stream succeeds and is queryable after flush; cold-start latency after a new version publish is ≤ 500 ms on a subsequent cold start (SnapStart restore).
