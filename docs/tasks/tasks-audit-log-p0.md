...
- [x] **P0-05a** Gradle setup and minimal Lambda handler (`service/app/`): rename base package from `org.example` → `com.marksayson.auditlogworker` (move source files under `src/main/kotlin/com/marksayson/auditlogworker/` and `src/test/kotlin/…`). Replace the `application` plugin with `com.github.johnrengelman.shadow` to produce a fat jar (required for Lambda deployment). Remove the `guava` dependency; add `aws-lambda-java-core`. Replace `App.kt` with a minimal `Handler` class implementing `RequestHandler<Map<String, Any>, String>` that returns a fixed string. Replace `AppTest.kt` with a test that instantiates `Handler` and asserts `handleRequest` returns a non-null response.
  *Test:* `./gradlew test` passes; `./gradlew shadowJar` produces `build/libs/app-all.jar`.

- [x] **P0-05b** `AuditEvent` Kotlin model and validator (`service/app/`): add a `model/` package with an `AuditEvent` data class mirroring all P0-02 fields (`event_id`, `event_time`, `enqueued_at`, `tenant_id`, `batch_id`, `work_type`, `item_id`, `item_type`, `status`, and nullable `skip_reason`, `error_code`, `error_reason`) and a `Status` enum (`PENDING`, `SUCCEEDED`, `FAILED`, `SKIPPED`). Add `AuditEventValidator.validate(event: AuditEvent)` throwing `IllegalArgumentException` on the first violation (blank required field or invalid status). Unit tests: a fully populated valid event passes; each required field individually blanked throws; an unrecognised status string throws.
  *Test:* `./gradlew test` passes.

- [x] **P0-05c** `FirehoseWriter` with exponential backoff + retry classification (`service/app/`): add `aws-sdk-kotlin` Firehose client (or AWS SDK for Java v2 `software.amazon.awssdk:firehose`). Implement `FirehoseWriter(streamName: String)` with `suspend fun putAuditEvent(event: AuditEvent)` that serialises the event to JSON, calls `PutRecord`, and retries on throttling exceptions (`ProvisionedThroughputExceededException`, `ServiceUnavailableException`) using exponential backoff with full jitter (base 100 ms, cap 30 s, max 5 attempts). All other exceptions are terminal — rethrow immediately without retry. Unit tests use a mock/fake Firehose client: throttle on first N calls then succeed → verify N+1 total calls and correct backoff ordering; terminal exception → verify single call and immediate rethrow; happy path → verify the serialised JSON body contains all required fields.
  *Test:* `./gradlew test` passes.

- [x] **P0-05d** Wire handler and CDK Lambda stack: update `Handler.handleRequest()` to deserialise the incoming map into an `AuditEvent`, call `AuditEventValidator.validate()`, then `FirehoseWriter.putAuditEvent()`, and return a success/failure indicator. In `AuditLogIngestStack`, add a `lambda.Function` pointed at `../../service/app/build/libs/app-all.jar` with `runtime: Runtime.JAVA_21`, `architecture: Architecture.ARM_64`, `snapStart: SnapStartConf.ON_PUBLISHED_VERSIONS`, and handler `com.marksayson.auditlogworker.Handler::handleRequest`. Publish a `lambda.Version` from the function (SnapStart only activates on published versions; callers must not target `$LATEST`). Grant the function role `firehose:PutRecord` on the P0-04 delivery stream. Add CDK tests: function has ARM_64, Java 21 runtime, SnapStart enabled; IAM policy includes `firehose:PutRecord`.
  *Deploy/test:* invoke the Lambda directly with a valid event JSON → within one buffer interval the record appears in S3; invoke with a malformed payload → Lambda returns an error without retrying. Measure cold-start latency on a subsequent cold invoke of the published version: should be ≤ 500 ms (SnapStart restore, not JVM init).
